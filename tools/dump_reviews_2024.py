"""
抓取台灣人壽 2024 年至今的 iOS + Android 評論，並匯出至 Excel。
"""
import os
import sys
...
def fetch_ios_reviews():
    ...
    print(f"[iOS] 正在抓取 {APP_NAME} 2024 至今評論 ...")
    app = AppStoreEntry(app_id=IOS_APP_ID, country="tw")
    all_reviews = []
    for r in app.reviews(limit=1000):
        try:
    ...
    print(f"[iOS] 共取得 {len(all_reviews)} 則從 2024 至今的評論")
    return all_reviews


def fetch_android_reviews():
    """使用 google_play_scraper 抓取 Android 評論。"""
    print(f"[Android] 正在抓取 {APP_NAME} 2024 至今評論 ...")
    all_reviews = []
    try:
        result, _ = reviews(
            ANDROID_APP_ID,
    ...
    print(f"[Android] 共取得 {len(all_reviews)} 則從 2024 至今的評論")
    return all_reviews
...
if __name__ == "__main__":
    ...
    print(f"\n[AI] 準備對 {len(all_data)} 則評論進行 AI 分類及正負面判定...")
    all_data = classify_reviews(all_data)

    df = pd.DataFrame(all_data)
    output_file = os.path.join(config.DATA_DIR, f"{APP_NAME}_2024至今評論_{datetime.now():%Y%m%d}.xlsx")
